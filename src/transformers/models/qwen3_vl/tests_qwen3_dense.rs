...
#[test]
fn qwen3_dense_forward_matches_python() {
    let path = match std::env::var("QWEN3_MODEL_PATH") {
        ...
        Err(_) => {
            ...
            let root = repo.get("config.json").expect("config.json download failed")
                .parent().unwrap().to_path_buf();
            repo.get("tokenizer.json").expect("tokenizer.json download failed");
            repo.get("model.safetensors").expect("model.safetensors download failed");
            root
        }
    };
    ...
    let model = super::load_text_model_from_dir(&path).expect("failed to load Rust TextModel");
    ...
    let py_vec: Vec<f32> = Python::with_gil(|py| -> PyResult<Vec<f32>> {
        ...
        py.run(c_str!(include_str!("tests_qwen3_dense_helper.py")), None, Some(&locals))?;
